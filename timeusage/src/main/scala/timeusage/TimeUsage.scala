...
object TimeUsage {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val categories = List(primary, working, leisure)
      .zipWithIndex
      .flatMap({case (prefixes, index) => prefixes.map((_, index))})

    val groupedColumns: Map[Int, List[Column]] = columnNames
      .map(columnName => {
        val matchedPrefix = categories.find({ case (prefix, category) => columnName.startsWith(prefix) })
        matchedPrefix.flatMap({case (prefix, categoryIndex) => Some(new Column(columnName), categoryIndex)})
      })
      .filter(_.isDefined)
      .map(_.get)
      .groupBy(_._2)
    ...
    def getColumnList(index: Int) = groupedColumns.getOrElse(index, List.empty)

    (getColumnList(0), getColumnList(1), getColumnList(2))
  }
